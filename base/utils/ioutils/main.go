...
func main() {
	...
	// dat, err := ioutil.ReadFile("http://s1.bdstatic.com/r/www/cache/static/home/js/nu_instant_search_08089ad.js")
	// check(err)
	...
	newFile, err := os.Create("test.js")
	if err != nil {
		log.Fatal(err)
	}
	defer newFile.Close()
	url := "http://s1.bdstatic.com/r/www/cache/static/home/js/nu_instant_search_08089ad.js"
	response, err := http.Get(url)
	defer response.Body.Close()
	// 将HTTP response Body中的内容写入到文件
	// Body满足reader接口，因此我们可以使用ioutil.Copy response.Body此处被读取了一次 于是下面的ioutil.ReadAll将不会再读取到任何内容
